...
function DonationContent() {
  ...
  // 1. Get bin from URL or Local Storage 📂
  useEffect(() => {
    // A. Check URL first
    let binCode = searchParams.get('bin');

    // B. If not in URL, check Local Storage (Backup)
    if (!binCode) {
      if (typeof window !== 'undefined') {
        const savedCode = localStorage.getItem('u4b_bin_id');
        if (savedCode) {
          console.log("📍 Retrieved Bin ID from storage:", savedCode);
          binCode = savedCode;
        }
      }
    } else {
      // C. If it IS in the URL, save it to storage (refresh the backup)
      if (typeof window !== 'undefined') {
        localStorage.setItem('u4b_bin_id', binCode);
      }
    }

    // D. If we found a code (from either source), fetch details
    if (binCode) {
      fetchBinByCode(binCode);
    }
...
const fetchBinByCode = async (binCode: string) => {
    try {
      ...
      // 1. Fetch data (Increased limit to 1000 to prevent missing bins)
      const response = await fetchWithAuth(`${API_URL}/bins?limit=1000`);
      const data = await response.json();

      if (data.success && data.data && data.data.bins) {
        
        const scannedInput = binCode.trim(); // Input from URL/QR or App Selection

        const bin = data.data.bins.find((b: Bin) => {
            // Get the ID string from DB (e.g. "L1975, L1983")
            const dbCodeString = b.bin_code || ""; 

            // Prepare list for QR checking (Split by comma)
            const codeList = dbCodeString.split(',').map((c: string) => c.trim());

            // CHECK A: Is it a specific QR scan? (e.g. "L1975" is inside ["L1975", "L1983"])
            const isQrMatch = codeList.includes(scannedInput);

            // CHECK B: Is it an App Selection? (e.g. "L1975, L1983" === "L1975, L1983")
            const isAppMatch = dbCodeString === scannedInput;

            // Return true if EITHER is correct
            return isQrMatch || isAppMatch;
        });

        if (bin) {
          console.log("✅ Match Found! Input:", scannedInput, "-> Location:", bin.location_name);
          setSelectedBin(bin);
          showSuccess(`Location detected: ${bin.location_name}`);

          // 💾 Save to storage safely
          if (typeof window !== 'undefined') {
             // If input was a long string (App Mode), grab the first code (e.g. "L1975") for cleanliness
             // If input was a single code (QR Mode), just use that.
             const cleanId = scannedInput.includes(',') ? (bin.bin_code || "").split(',')[0].trim() : scannedInput;
             
             if (cleanId) {
                localStorage.setItem('u4b_bin_id', cleanId);
             }
          }
        } else {
          console.error(`❌ No match found for "${scannedInput}"`);
          showError(`Bin location not found`);
        }
      }
    } catch (error) {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      mediaRecorder.start();
      setIsRecording(true);
      showInfo('Recording started! Max 15 seconds');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        showSuccess('🎉 Donation submitted! We will notify you via email once reviewed.');
        setTimeout(() => {
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {isRecording && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-red-600 font-bold">{recordingTime}s / 15s</span>
              </div>
            )}
          </div>
