...
export default function BinsPage() {
  ...
  const [canDonate, setCanDonate] = useState(true); // Default to true until checked
  ...
  // NEW: Check user eligibility on load
  useEffect(() => {
    const checkEligibility = async () => {
      try {
        ...
        const response = await fetch(`${API_URL}/donations/check-eligibility`, {
          headers: {
...
<button
  onClick={(e) => {
    if (canDonate) {
      e.stopPropagation();
      router.push(`/donation?bin=${bin.bin_code}`)// Only allow click if eligible
    }
  }}
  disabled={!canDonate} // 1. Disable the button
  className={`w-full mt-4 py-3 rounded-lg transition-colors flex flex-col items-center justify-center ${
    canDonate 
      ? 'bg-gradient-to-br from-[#A0BE6F] to-[#8FAF5F] hover:shadow-lg text-white'  // Active Style
      : 'bg-gray-400 text-white cursor-not-allowed'  // Cooldown Style
  }`}
>
  {/* 2. Change Text based on status */}
  <span className="font-semibold">
    {canDonate ? '📷 Start Donation Here' : 'Donation on Cooldown'}
  </span>
