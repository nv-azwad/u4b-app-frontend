...
interface Voucher {
  ...
  // We ignore expiry_date from here for the "Available" display
}
...
interface ClaimedVoucher {
  ...
  partner_name: string;
  discount_amount: string;
  expiry_date: string; // This is the fixed one from DB
}
...
export default function VoucherPage() {
  ...
  // New State: Dynamic Future Expiry for display
  const [dynamicExpiryDisplay, setDynamicExpiryDisplay] = useState<string>('');
  ...
  // Calculate "Today + 3 Months" for the Available Vouchers display
  const calculateDynamicExpiry = () => {
    const date = new Date();
    date.setMonth(date.getMonth() + 3);
    setDynamicExpiryDisplay(date.toLocaleDateString('en-GB', { // en-GB for DD/MM/YYYY
  ...
  const handleClaimVoucher = async (voucherId: number) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        fetchData(); // Refresh all data to move voucher to "My Vouchers"
      } else {
      ...
      <div className="px-6">
              ...
              <div className="flex-1">
                ...
                <p className={`text-sm ${
                  ...
                  {!eligibility.hasApprovedDonation && 
                    'You donation needs to be approved to claim your reward.'}
                  {eligibility.canClaim && 
                ...
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      {/* DYNAMIC DISPLAY: Uses the calculated future date */}
                      <span className="text-xs text-gray-500 flex items-center gap-1">
                        <Calendar size={12} />
                        Valid until {dynamicExpiryDisplay}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              {myVouchers.map((voucher) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-600">
                    ...
                    {/* FIXED DISPLAY: Uses the saved expiry from DB */}
                    <span>Expires: {voucher.expiry_date ? new Date(voucher.expiry_date).toLocaleDateString() : "N/A"}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-blue-50 border-l-4 border-blue-400 rounded-xl p-4 mt-6">
          ...
          <ul className="text-xs text-blue-700 space-y-1">
            ...
            <li>• Claim a voucher after approval</li>
            <li>• Use your voucher code at the partner store</li>
            <li>• Claim your new voucher every month (30 days)!</li>
          </ul>
        </div>
